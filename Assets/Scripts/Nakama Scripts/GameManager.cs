...
public class GameManager : MonoBehaviour
{
    ...
    private bool coinsDeductedForThisMatch = false;
    ...
    private async void Start()
    {
        ...
        if (GameRulesManager.currentSelectedGame_MatchType == GameRulesManager.MatchType.Multiplayer)
        {
            ...
            // Socket Handlers — only use OnRequestQuitMatch as the single path to QuitMatch
            NakamaConnection.Socket.Closed += () => UnityMainThreadDispatcher.Instance().Enqueue(() => OnRequestQuitMatch.Invoke());
    ...
    private async Task OnReceivedMatchmakerMatched(IMatchmakerMatched matched)
    {
        ...
        if (GameRulesManager.currentSelectedGame_MatchType == GameRulesManager.MatchType.Multiplayer && !coinsDeductedForThisMatch)
        {
            coinsDeductedForThisMatch = true;
    ...
    public async void FindMatch()
    {
        ...
        coinsDeductedForThisMatch = false;
    ...
    private async void HandleState(IMatchState matchState)
    {
        ...
        {
            Debug.LogError("HandleState exception: " + ex);
        }
    }
    ...
    public async void ExitSearching()
    {
       await NakamaConnection.CancelMatchmaking();
       await QuitMatch();
    }
    ...
    public async Task QuitMatch()
    {
        ...
        if(currentMatch != null && NakamaConnection.Socket != null && NakamaConnection.Socket.IsConnected)  await NakamaConnection.Socket.LeaveMatchAsync(currentMatch);
